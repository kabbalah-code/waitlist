"use client"

import { useState } from "react"
import { Send, Loader2, Check, AlertCircle, ExternalLink } from "lucide-react"

interface TelegramConnectSimpleProps {
  walletAddress: string
  userId: string
  onConnected: (username: string) => void
  onClose?: () => void
}

export function TelegramConnectSimple({ 
  walletAddress, 
  userId, 
  onConnected, 
  onClose 
}: TelegramConnectSimpleProps) {
  const [telegramUsername, setTelegramUsername] = useState('')
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const [success, setSuccess] = useState(false)

  const handleConnect = async () => {
    if (!telegramUsername.trim()) {
      setError('Please enter your Telegram username')
      return
    }

    let username = telegramUsername.trim()
    if (!username.startsWith('@')) {
      username = '@' + username
    }

    setLoading(true)
    setError('')

    try {
      const response = await fetch('/api/telegram/verify-direct', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          walletAddress,
          userId,
          telegramUsername: username
        })
      })

      const data = await response.json()

      if (data.success) {
        setSuccess(true)
        onConnected(username)
        setTimeout(() => {
          if (onClose) onClose()
        }, 2000)
      } else {
        setError(data.error || 'Failed to connect Telegram account')
      }
    } catch (err) {
      console.error('[Telegram] Connection error:', err)
      setError('Network error. Please try again.')
    } finally {
      setLoading(false)
    }
  }

  if (success) {
    return (
      <div className="p-6 bg-green-500/10 border border-green-500/30 rounded-lg">
        <div className="flex items-center gap-3 mb-4">
          <Check className="w-6 h-6 text-green-500" />
          <h3 className="text-lg font-bold text-green-400">Telegram Connected!</h3>
        </div>
        <p className="text-white/70 mb-2">
          Successfully connected {telegramUsername}
        </p>
        <p className="text-green-400 font-bold">
          +100 points earned! 🎉
        </p>
      </div>
    )
  }

  return (
    <div className="p-6 bg-[#0a0a0a] border border-[#0088cc]/30 rounded-lg">
      <div className="flex items-center gap-3 mb-4">
        <Send className="w-6 h-6 text-[#0088cc]" />
        <h3 className="text-lg font-bold text-white">Connect Telegram</h3>
      </div>

      <div className="space-y-4">
        <div className="p-4 bg-[#0088cc]/10 border border-[#0088cc]/30 rounded">
          <h4 className="text-white font-medium mb-2">Rewards:</h4>
          <ul className="text-sm text-white/70 space-y-1">
            <li>• <span className="text-[#FF9500]">100 points</span> + <span className="text-[#FF9500]">1 KCODE token</span></li>
            <li>• Verified account status</li>
            <li>• Access to Telegram tasks</li>
          </ul>
        </div>

        <div>
          <label className="block text-white/70 text-sm mb-2">
            Your Telegram Username
          </label>
          <div className="flex gap-2">
            <div className="relative flex-1">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-white/50">@</span>
              <input
                type="text"
                value={telegramUsername}
                onChange={(e) => setTelegramUsername(e.target.value)}
                placeholder="username"
                className="w-full bg-black/50 border border-[#0088cc]/30 pl-8 pr-4 py-3 text-white focus:outline-none focus:border-[#0088cc]"
                disabled={loading}
              />
            </div>
            <button
              onClick={handleConnect}
              disabled={loading || !telegramUsername.trim()}
              className="px-6 py-3 bg-[#0088cc] text-white font-bold hover:bg-[#0099dd] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
              {loading ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" />
                  Connecting...
                </>
              ) : (
                <>
                  <Send className="w-4 h-4" />
                  Connect
                </>
              )}
            </button>
          </div>
        </div>

        {error && (
          <div className="flex items-start gap-2 p-3 bg-red-500/10 border border-red-500/30 rounded">
            <AlertCircle className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" />
            <p className="text-red-400 text-sm">{error}</p>
          </div>
        )}

        <div className="p-3 bg-blue-500/10 border border-blue-500/30 rounded">
          <h4 className="text-blue-400 font-medium mb-2 flex items-center gap-2">
            <ExternalLink className="w-4 h-4" />
            How to find your username:
          </h4>
          <ol className="text-sm text-white/70 space-y-1 list-decimal list-inside">
            <li>Open Telegram app</li>
            <li>Go to Settings → Edit Profile</li>
            <li>Your username is shown there (without @)</li>
            <li>If you don't have one, create it first</li>
          </ol>
        </div>

        <p className="text-white/40 text-xs text-center">
          Make sure your Telegram username is public and correct
        </p>
      </div>
    </div>
  )
}